...
import { Music, LogOut, Calendar, Ticket, BookOpen, Video, Info, MessageSquare, LayoutGrid, AlertCircle, CheckCircle, User, ArrowRight, MapPin, X, Clock, FileText, Check, Bell, Megaphone } from 'lucide-react';
...
export default function StudentPortal({ user, logout, db, appId }) {
  ...
  const [announcements, setAnnouncements] = useState([]); // <-- NUEVO ESTADO PARA EL TABLÓN
  ...
  useEffect(() => {
    ...
    // --- NUEVO: ESCUCHADOR DEL TABLÓN DE AVISOS EN TIEMPO REAL ---
    const unsubAnnouncements = onSnapshot(collection(db, 'artifacts', appId, 'announcements'), (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      // Ordenamos para que los avisos más recientes salgan primero
            ...
            {announcements.length === 0 ? (
              ...
              <div className="space-y-4">
                {announcements.map(ann => {
                  // Determinamos los colores según el tipo de aviso (urgente, info, etc.)
